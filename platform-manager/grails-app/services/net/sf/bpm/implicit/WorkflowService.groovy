...
class WorkflowService {
	...
	def removeFromArrays(taskid, Map existingVars = [:], Map assignedVars = [:]) {
		...
		existingVars.each { k, v ->
			...
			if (k.contains('-') && 
				...
				!assignedVars.containsKey(k)) { // array component in assigned vars. check if array is reassigned and if index exists
				toremove.add(k)
			}
		}
	...
	def getVars(taskId, user = null, vars = [:], formdata = [:], taskdata = [:]) {
		...
		def r = ['_formdata': fd] // it contains the configuration of form fields of the tasks that have already been visited
		if (user)
			r['_user'] = user
		if (taskdata)
...
//		println "HISTORICS VARIABLES: $histvars"
		...
		hist.each { acid, h ->
			...
			try {
				...
				def hv = groupedhv?.collectEntries({k, v -> [k, v[-1]]}) ?: [:] // only one value per variable and task
...
//		println "DEFINITION: $definition"
		definition.each { k,v ->
			...
			else if (v.label && v.label ==  data[v.name]) {  // enum post process and other types that do have a label
				data[v.name].label = v.label
			}
	...
	def getProcessDefinition(String processDefinitionId) {
		repositoryService.createProcessDefinitionQuery().processDefinitionId(processDefinitionId).singleResult()
	}
	
	def getStartFormData(String taskid) {
		formService.getStartFormData(taskid)
	}
